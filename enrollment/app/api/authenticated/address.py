"""Etapa address — endereço; avança profile → address."""
...
from app.dependencies import require_profile
...
@router.get("/address", response_model=AddressGetResponse, summary="Endereço atual")
async def get_address(external_id=require_profile()):
...
@router.get(
    ...
    summary="Consulta CEP via serviço address",
)
async def check_cep(cep: str, external_id=require_profile()):
    with upstream_errors():
...
@router.post("/address", response_model=AddressPostResponse, summary="Envia endereço")
async def post_address(
    payload: AddressPostRequest,
    background_tasks: BackgroundTasks,
    session: AsyncSession = Depends(get_session),
    external_id=require_profile(),
):
    with upstream_errors():
