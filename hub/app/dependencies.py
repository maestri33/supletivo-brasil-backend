...
Espelha `staff/app/dependencies.py`. O hub expoe endpoints de escrita
autenticados que so podem ser chamados por um token cujas roles tenham
intersecao com `settings.staff_roles` (admin, staff).
"""
...
async def get_current_external_id(
    ...
    """Valida JWT RS256, exige `exp`/`roles`/`external_id`, e checa role in STAFF_ROLES."""
