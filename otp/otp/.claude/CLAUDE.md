...
- **Rede:** está numa **DMZ**. Não há firewall protegendo este serviço de
  ...
  pode pular auth, CORS aberto. (Já existe **rate limit** por `external_id` na
  geração de OTP — `app/services/rate_limit.py`.) Auth é o próximo passe
  explícito de "agora trava isso".
