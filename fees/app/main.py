"""Entrypoint FastAPI — serviço `fees` (taxas de matrícula)."""
...
fsl.setup_logging(json_logs=(settings.env != "dev"), log_level=settings.log_level)
logger = fsl.get_logger()


@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("fees_starting", env=settings.env, schema=settings.database_schema)
    yield
