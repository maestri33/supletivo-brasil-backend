...
async def _check_whatsapp(phone: str) -> bool:
    ...
    async with httpx.AsyncClient() as http:
        ...
        try:
            ...
            log.warning("whatsapp.check_failed", phone=_mask_phone(phone), error=str(exc))
            return False
...
async def normalize_and_validate(phone: str) -> str:
    ...
    if not valid and len(raw) == 10:
        ...
        log.info("phone.trying_with_9", original=_mask_phone(raw), attempt=_mask_phone(with_9))
