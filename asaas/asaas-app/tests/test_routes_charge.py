...
def _seed_customer(db, ext_id="aluno_42", asaas_id="cus_42"):
    row = Customer(
        external_id=ext_id,
        asaas_id=asaas_id,
        name="Maria",
...
def test_post_charge_pix_falta_customer_400(client, seeded_apikey, fake_asaas):
...
def test_post_charge_pix_com_customer_existente(db, client, seeded_apikey, fake_asaas):
    _seed_customer(db)
...
def test_post_charge_pix_inline_payer_cria_customer(db, client, seeded_apikey, fake_asaas):
    fake_asaas.find_customer_by_external_reference.return_value = None
    ...
    assert r.status_code == 200
    db.expire_all()
    assert db.query(Customer).filter_by(external_id="aluno_novo").one().asaas_id == "cus_brand_new"


def test_post_charge_pix_amount_zero_422(client, seeded_apikey, fake_asaas):
...
def test_post_charge_pix_idempotencia(db, client, seeded_apikey, fake_asaas):
    _seed_customer(db)
    fake_asaas.create_payment.return_value = _mock_charge("pay_R1")
    fake_asaas.get_payment_pix_qr_code.return_value = _mock_qr()
    payload = {
    ...
    r1 = client.post("/api/v1/charge/pix", json=payload)
    assert r1.status_code == 200
    r2 = client.post("/api/v1/charge/pix", json=payload)
...
def test_get_charge_completo(db, client, seeded_apikey, fake_asaas):
    _seed_customer(db)
    fake_asaas.create_payment.return_value = _mock_charge("pay_R1")
    fake_asaas.get_payment_pix_qr_code.return_value = _mock_qr()
    created = client.post(
    ...
    r = client.get(f"/api/v1/charge/{pid}")
    assert r.status_code == 200
    assert r.json()["payment_id"] == pid
    assert r.json()["pix"]["encoded_image"] == "PNG_BASE64_FAKE"


def test_get_charge_status_light(db, client, seeded_apikey, fake_asaas):
    _seed_customer(db)
    fake_asaas.create_payment.return_value = _mock_charge("pay_R1")
    fake_asaas.get_payment_pix_qr_code.return_value = _mock_qr()
    created = client.post(
    ...
    r = client.get(f"/api/v1/charge/{pid}/status")
    assert r.status_code == 200
    body = r.json()
...
def test_get_charge_not_found_404(client, seeded_apikey):
    r = client.get("/api/v1/charge/pay_nao_existe")
    assert r.status_code == 404
    r2 = client.get("/api/v1/charge/pay_nao_existe/status")
...
def test_delete_charge_pending(db, client, seeded_apikey, fake_asaas):
    _seed_customer(db)
    fake_asaas.create_payment.return_value = _mock_charge("pay_R1")
    fake_asaas.get_payment_pix_qr_code.return_value = _mock_qr()
    created = client.post(
    ...
    r = client.delete(f"/api/v1/charge/{pid}")
    assert r.status_code == 200
    assert r.json()["status"] == "CANCELLED"


def test_delete_charge_404(client, seeded_apikey):
    r = client.delete("/api/v1/charge/pay_nao_existe")
    assert r.status_code == 404
...
def test_webhook_payment_received_atualiza_charge(
    ...
    db.expire_all()
    p = db.query(Payment).filter_by(payment_id=pid).one()
    assert p.status == "PAID"


def test_webhook_payment_overdue_atualiza_charge_expired(
    db, client, seeded_apikey, seeded_token, fake_asaas
):
    _seed_customer(db)
...
def test_webhook_transfer_event_nao_afeta_charge(
    db, client, seeded_apikey, seeded_token, fake_asaas
):
