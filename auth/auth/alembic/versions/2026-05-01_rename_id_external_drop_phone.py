...
def upgrade() -> None:
    op.execute(sa.text("ALTER TABLE auth.identity_roles DROP CONSTRAINT identity_roles_identity_id_fkey"))
    op.alter_column(
        'identities', 'id_external',
        new_column_name='external_id', schema='auth'
    )
    op.execute(sa.text(
...
def downgrade() -> None:
    ...
    op.alter_column(
        ...
        new_column_name='id_external', schema='auth'
    )
    op.execute(sa.text(
        "ALTER TABLE auth.identity_roles ADD CONSTRAINT identity_roles_identity_id_fkey "
